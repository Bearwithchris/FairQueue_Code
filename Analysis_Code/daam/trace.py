...
class DiffusionHeatMapHooker(AggregateHooker):
    ...
    def compute_global_heat_map(self, prompt=None, factors=None, head_idx=None, layer_idx=None, inference_step_range=None, normalize=False):
        # type: (str, List[float], int, int, List[int], bool) -> GlobalHeatMap
        """
        Args:
            inference_step_range: Restrict the application to heat maps with these inference steps. If `None`, use all steps.
        see others in 'compute_global_heat_map_original'

        all 'step' and 'inference_step_range' are added, they do not exist in the original code
        ...
        with auto_autocast(dtype=torch.float32):
            for (step, factor, layer, head), heat_map in heat_maps:
                if factor in factors and (head_idx is None or head_idx == head) and (layer_idx is None or layer_idx == layer) and (inference_step_range is None or step>=inference_step_range[0] and step<inference_step_range[1]):
            ...
            try:
                ...
                if head_idx is not None or layer_idx is not None or inference_step_range is not None:
...
class UNetCrossAttentionHooker(ObjectHooker[Attention]):
    ...
    def __call__(
        ...
        batch_size, sequence_length, _ = hidden_states.shape  # 'hidden_states' reflects the latent, see 'sample' in forward pass of 'UNet2DConditionModel' in 'unet_2d_condition'
        ...
        key = attn.to_k(encoder_hidden_states)  # 'encoder_hidden_states' is the prompt_embeds, see 'prompt_embeds' in 'pipeline_stable_diffusion'
        ...
        attention_probs = attn.get_attention_scores(query, key, attention_mask)  # softmax will change the distribution of attention score
        ...
        if attention_probs.shape[-1] == self.context_size and factor != 8:
            ...
            for head_idx, heatmap in enumerate(maps):
                self.heat_maps.update(inference_step, factor, self.layer_idx, head_idx, heatmap)
